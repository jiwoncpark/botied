...
def _mes_callback(
        ...
        estimation_type: str, *args, **kwargs):
    """Wrapper around `qLowerBoundMultiObjectiveJointEntropySearch`
    ...
    optimal_pf_samples = optimal_pf_samples[is_nd]
    hypercell_bounds = compute_sample_box_decomposition(
        optimal_pf_samples.unsqueeze(0))
    mes_lb_obj = qLowerBoundMultiObjectiveMaxValueEntropySearch(
        model=model,
        pareto_fronts=optimal_pf_samples.unsqueeze(0),
        hypercell_bounds=hypercell_bounds,
        estimation_type=estimation_type,
    )
    scores = mes_lb_obj(x_cand.unsqueeze(-2))
    return scores
...
class MES(BaseAcquisition):
    ...
    def select_subset(
        ...
        for _ in tqdm(range(q_batch_size), desc=self.__class__.__name__):
            ...
            scores[exclude] = -np.inf  # acq value can be negative for MES
            # Find best in batch
    ...
    def optimize(
        ...
        of the MES acquisition function, and returns a new candidate and
        observation."""
        total_retries = 100
