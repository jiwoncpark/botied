...
def pes_callback(f_cand, model, x_cand, *args, **kwargs):
    ...
    # f_nd = f_baseline[is_nd]
    x_nd = x_cand[is_nd]
    pes_obj = qMultiObjectivePredictiveEntropySearch(
        model=model, pareto_sets=x_nd.unsqueeze(0))
    scores = pes_obj(x_cand.unsqueeze(-2))
    return scores


def select_pes(y_pool, y_known, q_batch_size, ref_point, model, x_pool,
